...
THNN_(FeatureLPPooling_updateOutput)(
  ...
  ntype* inputP = THTensor_(data)(input);
  ntype* outputP = THTensor_(data)(output);
...
THNN_(FeatureLPPooling_updateGradInput)(
  ...
  ntype* gradOutputP = THTensor_(data)(gradOutput);
  ntype* gradInputP = THTensor_(data)(gradInput);
  ntype* outputP = THTensor_(data)(output);
  ntype* inputP = THTensor_(data)(input);
      ...
      for (opt2 = 0; opt2 < FEATURE_LP_CAST_TYPE inputDesc.size[3]; ++opt2) {
        for (outputFeature = 0;
          ...
          for (i = 0; i < width; ++i) {
            ...
            ntype v = gradOutputV * pow(inputV / outputV, power - (accntype) 1);
