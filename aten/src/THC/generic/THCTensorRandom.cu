...
THC_API void THCTensor_(uniform)(THCState* state, THCTensor *self_, double a, double b)
{
  ...
  ntype *data = THCTensor_(data)(state, self);
...
THC_API void THCTensor_(normal_means)(THCState *state, THCTensor *self, THCTensor *means, double stddev) {
  ...
  THCTensor_(cadd)(state, self, self, ScalarConvert<int, ntype>::to(1), means);
}
...
THC_API void THCTensor_(normal_stddevs)(THCState *state, THCTensor *self, double mean, THCTensor *stddevs)
{
  ...
  THCTensor_(add)(state, self, self, ScalarConvert<double, ntype>::to(mean));
}
...
THC_API void THCTensor_(logNormal)(THCState* state, THCTensor *self_, double mean, double stdv)
{
  ...
  generateLogNormal<ntype><<<NUM_BLOCKS, BLOCK_SIZE, 0, THCState_getCurrentStream(state)>>>(
      gen->gen_states, size, data, mean, stdv);
...
void THCTensor_(renormRows)(struct THCState* state,
  ...
  renormRowsL1<ntype>
    <<<grid, block, block.x * sizeof(ntype),
                                      ...
                                      int with_replacement)
{
  ...
  if (n_sample == 1 && maxShared >= requiredShared) {
    ...
    sampleMultinomialOnce<ntype, accntype>
      <<<grid, block,
         requiredShared,
...
THC_API void THCTensor_(multinomialAliasSetup)(THCState *state, THCTensor *_probs, THCudaLongTensor *_J, THCTensor *_q){
  ...
  ntype one = ScalarConvert<int64_t, ntype>::to(1);
  ...
  ntype q_max = THCTensor_(maxall)(state, _q);
  condDiv<<<
    inputBlockDim, BLOCK_SIZE, 0, THCState_getCurrentStream(state)>>>(
                      THCTensor_(data)(state, _q),
                      THCudaLongTensor_data(state, _J),
...
#else
GENERATE_KERNEL1(generate_bernoulli, ntype, double p, float, curand_uniform, (ScalarConvert<bool, ntype>::to(x <= p)))
#endif
...
{                                                                              \
  ...
  ntype *result_data = THCTensor_(data)(state, self);                           \
...
#else
GENERATE_KERNEL1(generate_geometric, ntype, double p, float, curand_uniform, (ScalarConvert<float, ntype>::to(ceilf(logf(x) / log(1-p)))))
#endif
...
GENERATE_KERNEL2(generate_random, ntype, int32_t base, uint32_t range, uint32_t, curand, (ntype)(x % range + base))
GENERATE_KERNEL2(generate_random_64, ntype, int64_t base, uint64_t range, uint64_t, CURAND64, (ntype)(x % range + base))
#elif defined(THC_NTYPE_IS_HALF)
GENERATE_KERNEL2(generate_random, ntype, int32_t base, uint32_t range, uint32_t, curand, (ScalarConvert<uint32_t, ntype>::to(x % range + base)))
#else
GENERATE_KERNEL2(generate_random, ntype, int32_t base, uint32_t range, uint32_t, curand, (ntype)(x % range + base))
#endif
...
THC_API void THCTensor_(random)(THCState* state, THCTensor *self_)
{
  ...
  generate_random_64<<<NUM_BLOCKS, BLOCK_SIZE, 0, THCState_getCurrentStream(state)>>>(
      gen->gen_states, size, data, 0ULL, static_cast<uint64_t>(std::numeric_limits<ntype>::max()) + 1);
#else
  generate_random<<<NUM_BLOCKS, BLOCK_SIZE, 0, THCState_getCurrentStream(state)>>>(
      gen->gen_states, size, data, 0UL, static_cast<uint32_t>(std::numeric_limits<ntype>::max()) + 1);
#endif
