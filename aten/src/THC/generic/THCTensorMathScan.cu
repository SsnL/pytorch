    ...
    BinaryFunction binary_op)
{
  ...
  thrust::device_ptr<ntype> dst_data(THCTensor_(data)(state, dst));
                                       ...
                                       ntype init, BinaryOp binary_op)
{
  ...
  THCTensor_kernel_scanOuterDim<ntype><<<grid, threads, 0, THCState_getCurrentStream(state)>>>(
                                           ...
                                           BinaryFunction binary_op)
{
  ...
  THCTensor_kernel_scanInnermostDim<ntype, 16, 32><<<grid, threads, 0, THCState_getCurrentStream(state)>>>(
...
void THCTensor_(scanDim)(THCState *state, THCTensor *self_, THCTensor *src,
                         int dimension, ntype init, BinaryFunction binary_op)
{
...
void THCTensor_(cumsum)(THCState *state, THCTensor *self, THCTensor *src, int dimension)
{
  ...
  return THCTensor_(scanDim)(state, self, src, dimension,
                             ScalarConvert<float, ntype>::to(0.0), AddOp<ntype>());
}
...
void THCTensor_(cumprod)(THCState *state, THCTensor *self, THCTensor *src, int dimension)
{
  THCAssertSameGPU(THCTensor_(checkGPU)(state, 2, self, src));
  return THCTensor_(scanDim)(state, self, src, dimension,
                             ScalarConvert<float, ntype>::to(1.0), MulOp<ntype>());
}
