           ...
           bool reduce)
{
  ...
  if (!reduce) {
    ...
    THC_pointwiseApply3(state, input, target, output,
                        smoothl1_updateOutput_no_reduce_functor<ntype>());
    return;
  }
  ...
  accntype sum = thrust::inner_product(
    ...
    thrust::plus<accntype>(), smoothl1_functor<ntype, accntype>()
  );
           ...
           bool reduce)
{
  ...
  if (!reduce) {
    ...
    THC_pointwiseApply3(state, input, target, gradInput,
                        smoothl1_updateGradInput_no_reduce_functor<ntype>());
  ...
  ptrdiff_t size = THCTensor_(nElement)(state, input);
  ntype norm = ScalarConvert<accntype, ntype>::to(sizeAverage ? accntype(1)/size : accntype(1));
  ...
  thrust::transform(
    ...
    smoothl1_updateGradInput_functor<ntype>(norm, THCTensor_(get1d)(state, gradOutput, 0))
  );
