...
void THNN_(TemporalReflectionPadding_updateOutput)(THCState *state,
  ...
  THCDeviceTensor<ntype, 3> devInput;
  THCDeviceTensor<ntype, 3> devOutput;

  if (numInputDims == 2) {
    THCTensor_(resize2d)(state, output, numPlanes, outputW);

    devInput = toDeviceTensor<ntype, 2>(state, input).upcastOuter<3>();
    devOutput = toDeviceTensor<ntype, 2>(state, output).upcastOuter<3>();
  } else {
    THCTensor_(resize3d)(state, output, numBatch, numPlanes, outputW);

    devInput = toDeviceTensor<ntype, 3>(state, input);
    devOutput = toDeviceTensor<ntype, 3>(state, output);
  }
...
void THNN_(TemporalReflectionPadding_updateGradInput)(
  ...
  THCDeviceTensor<ntype, 3> devGradInput;
  THCDeviceTensor<ntype, 3> devGradOutput;

  if (numInputDims == 2) {
    devGradInput = toDeviceTensor<ntype, 2>(state, gradInput).upcastOuter<3>();
    devGradOutput = toDeviceTensor<ntype, 2>(state, gradOutput).upcastOuter<3>();
  } else {
    devGradInput = toDeviceTensor<ntype, 3>(state, gradInput);
    devGradOutput = toDeviceTensor<ntype, 3>(state, gradOutput);
  }
