...
void THNN_(SpatialSubSampling_updateOutput)(
           ...
           int dW, int dH)
{
  ntype *weight_data = THCTensor_(data)(state, weight);
  ntype *bias_data = THCTensor_(data)(state, bias);
  ntype *output_data;
  ntype *input_data;
  ...
  if (input->nDimension == 3) {
    ...
    subsample<ntype, accntype> <<<blocks, threads, 0, THCState_getCurrentStream(state)>>> (
           ...
           accntype scale)
{
  ...
  if (input->nDimension == 3) {
    ...
    ntype *gradWeight_data = THCTensor_(data)(state, gradWeight);
    ntype *gradBias_data = THCTensor_(data)(state, gradBias);
    gradOutput = THCTensor_(newContiguous)(state, gradOutput);
    ntype *gradOutput_data = THCTensor_(data)(state, gradOutput);
    ntype *input_data;
    ...
    subgradweight<ntype, accntype> <<<blocks, threads, 0, THCState_getCurrentStream(state)>>> (
