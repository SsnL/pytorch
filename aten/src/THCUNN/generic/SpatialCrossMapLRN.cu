...
void LRNforward(THCState* state, THCTensor* input, THCTensor* output,
    THCTensor* scale, int local_size, accntype alpha_, accntype beta_, accntype k_)
{
  ntype alpha = ScalarConvert<accntype, ntype>::to(alpha_);
  ntype beta = ScalarConvert<accntype, ntype>::to(beta_);
  ntype k = ScalarConvert<accntype, ntype>::to(k_);
  ...
  LRNFillScale<ntype, accntype> <<<GET_BLOCKS(n_threads), CUDA_NUM_THREADS, 0, THCState_getCurrentStream(state)>>>(
...
void LRNbackward(THCState* state, THCTensor* input, THCTensor* output,
    ...
    int local_size, accntype alpha_, accntype beta_, accntype k_)
{
  ntype alpha = ScalarConvert<accntype, ntype>::to(alpha_);
  ntype beta = ScalarConvert<accntype, ntype>::to(beta_);
  ntype k = ScalarConvert<accntype, ntype>::to(k_);
  ...
  int n_threads = batchSize * imsize_h * imsize_w;
  LRNComputeDiff<ntype, accntype> <<<GET_BLOCKS(n_threads), CUDA_NUM_THREADS, 0, THCState_getCurrentStream(state)>>>(
      ...
      local_size, -beta, ScalarConvert<int, ntype>::to(2) * alpha * beta / local_size,
