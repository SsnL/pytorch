           ...
           THCTensor *randomSamples)
{
  ...
  THCDeviceTensor<ntype, 4> devInput;
  THCDeviceTensor<ntype, 4> devOutput;
  ...
  if (numInputDims == 3) {
    ...
    devInput = toDeviceTensor<ntype, 3>(state, input).upcastOuter<4>();
    devOutput = toDeviceTensor<ntype, 3>(state, output).upcastOuter<4>();
    ...
    devInput = toDeviceTensor<ntype, 4>(state, input);
    devOutput = toDeviceTensor<ntype, 4>(state, output);
...
#define SFMP_UPDATE_OUTPUT(POOL_W)                                      \
  SpatialFractionalMaxPooling_updateOutput<POOL_W, ntype, accntype>       \
           ...
           THCIndexTensor *indices)
{
  ...
  THCDeviceTensor<ntype, 4> devGradInput;
  THCDeviceTensor<ntype, 4> devGradOutput;
  THCDeviceTensor<THCIndex_t, 4> devIndices;

  /* backprop */
  if (numInputDims == 3) {
    devGradInput = toDeviceTensor<ntype, 3>(state, gradInput).upcastOuter<4>();
    devGradOutput = toDeviceTensor<ntype, 3>(state, gradOutput).upcastOuter<4>();
    devIndices = toDeviceTensor<THCIndex_t, 3>(state, indices).upcastOuter<4>();
  } else {
    devGradInput = toDeviceTensor<ntype, 4>(state, gradInput);
    devGradOutput = toDeviceTensor<ntype, 4>(state, gradOutput);
    devIndices = toDeviceTensor<THCIndex_t, 4>(state, indices);
  }
