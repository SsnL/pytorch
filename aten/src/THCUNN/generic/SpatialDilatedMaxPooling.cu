           ...
           bool ceil_mode)
{
  ...
  MaxPoolForward<ntype, accntype> <<< GET_BLOCKS(count), CUDA_NUM_THREADS, 0, THCState_getCurrentStream(state) >>>
      (count, input_data,
           ...
           bool ceil_mode)
{
  ...
  MaxPoolBackward<ntype, accntype> <<< grid, BACKWARD_THREADS, 0, THCState_getCurrentStream(state) >>>
      (count,
